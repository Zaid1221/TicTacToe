...
public class ButtonGridAndTextView extends GridLayout
{
    ...
    public void setStatusText( String text ) //Sets the satus at hte bottom of te screen
    {
        status.setText( text );
    }

    public void setStatusBackgroundColor( int color ) //sets the color of the status at the botoom of the screen
    {
        status.setBackgroundColor( color );
    }

    public void setButtonText( int row, int column, String text ) //sets the button text
    {
        buttons[row][column].setText( text );
    }

    public boolean isButton( Button b, int row, int column ) //sets button in place
    {
        return ( b == buttons[row][column] );
    }

    public void resetButtons( ) //resets all buttons to empty for playing again
    {
        for (int row = 0; row < side; row++)
        {
            for (int col = 0; col < side; col++)
    ...
    public void enableButtons( boolean enabled ) //enables the during play so moves can be made
    {
        for( int row = 0; row < side; row++ )
        {
            for (int col = 0; col < side; col++)
